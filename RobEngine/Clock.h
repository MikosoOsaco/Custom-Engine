...
public:
	...
	*/
	static inline double getCurrentSeconds()
	{
		return ticksToSeconds(getCurrentTicks());
	}

	static inline int getCurrentMinutes()
	{
		return getCurrentSeconds() / 60;
	}

	static inline int getCurrentHours()
	{
		return getCurrentMinutes() / 60;
	}
